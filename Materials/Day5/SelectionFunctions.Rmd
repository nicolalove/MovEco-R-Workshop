...
Niki checks out data:
```{r}
zebra <- read_csv("data_files/Zebra_Used_LatLong_Full.csv")# just check it out
View(zebra)
zebra %>%
  distinct(ID)%>%
  tally() # checking how many individuals are present in the 
```


```{r}
zebra <- read_csv("data_files/Zebra_Used_LatLong_Full.csv") %>% 
  dplyr::select(ID = Name, 4:6) %>% #selecting date, lat & long
  st_as_sf(., coords = 3:4, crs = "+init=epsg:4326") %>%  #creating simple features object from lat&long columns, define crs
  st_transform("+init=epsg:32733") %>% # convert coordinates to southern hemi/ SA 
  mutate(timestamp = lubridate::mdy_hm(Date)) # fix dates to month day year - hour min
...
zebra %>% 
  ...
  dplyr::select(-Date) %>% 
  mutate(Used = 1) -> zebra #wtf is used? == > ah it's habitat used by animal
```
...
# as(zebra, "Spatial") forces the zebra dataframe to be a spatial data frame - kernelUD takes "SpatialPointsDataFrame"
...
# ultimate goal: want to create matrix of available vs. actually used area, so must build areas not used by animal
#create availability 
zebra_ids <- unique(zebra$ID)
availables <- list()
for(i in 1:length(zebra_ids)){ 
  st_sample(st_as_sf(zebra_hr)[i,], 10*nrow(filter(zebra, ID == zebra_ids[i]))) %>% # sample individual UDs - 1:10 ratio; population UD
  st_sf(geometry = .) %>%
    mutate(ID = zebra_ids[i], 
          timestamp = NA, 
          Used = 0) -> availables[[i]] #habitat not used by animal
}
...
mapview(list(roads, zebra_hr)) #had to add list(), originally had "homerange" as the second layer but it's is a mapview map itself, can't put a map in as an argument 
primary_roads <- filter(roads, TYPE %in% c("Tar", "Gravel"))
mapview(list(primary_roads, zebra_hr))
...
overly correlated. If two variables are highly correlated with one another it makes it hard to distinguish their individual effects on the response variable - in this case, the use of a habitat type. 
...
People debate how to deal with multicollinearity but a standard *rule of thumb is to to drop variables that have a correlation coefficient > .7*
...
Often we want to interpolate a selection surface from our best model. To do this you must ensure first that you have complete data layers across you entire area of interest. Since our NDVI layer covers our entire zone we need not recreate that, but for our roads and water variables we need to create layers for distance to roads or distance to water across our entire regions.  We are going to show you below how to calculate this in the below chunk but as this takes a long time to process, we will pull in pre-calculated 
...
# st_make_grid creates a square or hexagonal a grid and calculate distances
# n = number of grid cells in x and y direction (columns, rows)
...
# st_distance: Compute Euclidian distance between pairs of geometries -> the area or the length of a set of geometries.
...
# x = matrix
# margin = a vector giving the subscripts which the function will be applied over. So for a matrix, 1 = rows, 2 = columns, c(1,2) is rows & columns
# fun = function to be applied
dist_raster[] = apply(dist_water_grid, 2, min)

#plot two lines together
plot(dist_raster)
