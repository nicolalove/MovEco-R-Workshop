...
Often, vector data is stored as "shapefiles" (.shp), like the `mapview` plots we made with the albatross data in Movement_Data_Path_Metrics.Rmd.

Vector Data == **RELOCATIONS/TRAJECTORIES**

## Raster Data
Raster models are a representation of the world as a surface divided 
into a regular grid of cells.

Raster Data == **HABITAT HEAT MAPS**

![Rasters divide the world into a regular grid](rasterpic.png)
...
![Rasters are better used for continuous data like temperature, elevation, or landcover type](rasterex.png)
...
Two main differences comparing to a regular `data.frame` object are spatial metadata (`geometry type`, `dimension`, `bbox`, `epsg (SRID)`, `proj4string`) and additional column - typically named `geom` or `geometry`.
...
Now let's use the `st_read` function to read both files in separately. Note thatwithin the `sf` library most commands begin with the "st" prefix. 
...
If we are ever curious about just the dataframe or just the geometry separately, we can use `st_geometry` commands.  By setting the geometry to 'NULL' using `st_set_geometry`, a `sf` object returns to a simple data.frame. Calling the `st_geometry` from an sf object would extract just the spatial attributes turning this into a "simple feature collection" or "sfc".  
...
points <- st_sample(HK_districts, 25)
mapview(st_sample(HK_districts, 25))

#icon: http://leafletjs.com/examples/custom-icons/
#fishIcon <- makeIcon("images/lumpsucker.jpg", 18,18)
#mapview(HK_regions)@map %>% addTiles %>%  addMarkers(data = points)

#mapview(points, map, cex=3, color="red") #this doesn't work: Error: $ operator not defined for this S4 class
mapview(list(points, HK_districts), cex = 3, color = "red") # my addition
```
...
HK_trim <- HK_boundary %>%
  dplyr::select(ID_0, ISO, NAME_LO, SOVEREI, FIPS, ISON, POP2000, SQKM) # had to add dlyr:: because got unused argument error -> MASS package also loaded (has select function).
...
```{r mutate}
HK_trim %>%
  mutate(POP_per_SQKM = POP2000/SQKM) %>% pull(POP_per_SQKM) # extracts single column (there's only one row so only has one element )
```
...
library(raster)
library(knitr)
# this data is no longer available, just reviewed the code
...
# unzip("data_files/Landcover.zip", "data_files/") #can't find this data, has probably been moved recently


# land_cover <- raster("data_files/LCType.tif")
# plot(land_cover)
# crs(land_cover) output: ## CRS arguments:
##  +proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0
```

![plot(landcover) output](plotlandcover.png)
...
plot(HK_lc_proj)
```
![plot(HK_landcover) output](HK_landcover.png)
![plot(HK_lc_proj) output](HK_lc_proj.png)
...
```{r}
# library(velox) #velox removed/archived
library(fasterize)
...
# think you can do the same with packages fasterize or raster
